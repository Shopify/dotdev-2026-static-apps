 ...
 * @property {boolean} show_on_faq_page
 */

/** @typedef {FAQ & { id: string }} FAQSummary */
...
/** @type {Map<string, FAQ>} */
const primedFAQs = new Map();

export function primeFAQ(id, faq) {
  primedFAQs.set(id, { ...faq });
}

export function getPrimedFAQ(id) {
...
export async function listFAQs() {
  ...
  const faqs = json.data.metaobjects.edges.map(({ node }) => ({
    id: node.id,
